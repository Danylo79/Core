...
public class KillMessageMenu extends Menu {
    public KillMessageMenu() {
        super("&aKill Messages", false, 54);
    }
    ...
    public void addKillMessages(Profile profile) {
        for (int i = 0; i < inventory.getSize(); i++) {
            try {
                if (CosmeticManager.getCosmetics(CosmeticType.KILL_MESSAGE).get(i) == null) {
                    break;
                }
                KillMessage k = (KillMessage) CosmeticManager.getCosmetics(CosmeticType.KILL_MESSAGE).get(i);
    ...
    public void onClick(Profile player, int slot) {
        try {
            ...
            } else if (CosmeticManager.getCosmetics(CosmeticType.KILL_MESSAGE).get(slot) != null) {
                Cosmetic m = CosmeticManager.getCosmetics(CosmeticType.KILL_MESSAGE).get(slot);
