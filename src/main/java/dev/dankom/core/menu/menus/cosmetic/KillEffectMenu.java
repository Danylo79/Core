...
public class KillEffectMenu extends Menu {
    ...
    public void addKillMessages(Profile profile) {
        for (int i = 0; i < inventory.getSize(); i++) {
            try {
                if (CosmeticManager.getCosmetics(CosmeticType.KILL_EFFECT).get(i) == null) {
                    break;
                }
                KillMessage k = (KillMessage) CosmeticManager.getCosmetics(CosmeticType.KILL_EFFECT).get(i);
