        ...
        if (command.getName().equalsIgnoreCase("cosmetics")) {
            if (args.length > 0) {
                if (args[0].equalsIgnoreCase("emoji")) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&aThe emoji's available to " + Rank.MVP_PLUS_PLUS.getDisplay() + " &aare:"));
                    for (Emoji e : EmojiRepository.getEmojis()) {
                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&a" + e.getText() + " &f-&a " + e.getReplacement()));
                    }
                }
            } else {
